...
const TeamRoster = (props) => {
  ...
  const getProjectID = async (user_id) => {
    try {
      const response = await fetch(
        `http://localhost:3001/users/${user_id}/team`
      );
  ...
  const getTeamRoster = async (project_id) => {
    try {
      const response = await fetch(
        `http://localhost:3001/projects/${project_id}/team`
      );
      const jsonData = await response.json();
      setTeam(jsonData);
    } catch (err) {
  ...
  const getAllDevelopers = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/users/role/Developer`
      );
      const jsonData = await response.json();
  ...
  const getAllManagers = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/users/role/Project%20Manager`
      );
      const jsonData = await response.json();
  ...
  const getMatches = () => {
    ...
    matches.map((each) =>
      allDevelopers.map((developer) => {
        each.user_id === developer.user_id
  ...
  const handleDeleteMember = async (id) => {
    try {
      ...
      await fetch(
        `http://localhost:3001/projects/${projectID}/team/${id}`,
        requestOptions
      )
  ...
  const getDollarFigure = (amount) => {
    ...
    output.includes(".")
      ? output.split(".")[1].length === 0
        ? (output += "00")
        : output.split(".")[1].length === 1
        ? (output += "0")
        : null
      : (output += ".00");
    return output;
